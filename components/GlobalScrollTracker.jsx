    ...
    >
      ...
      {sections.map((s, i) => {
        ...
        return (
          <div
            key={s.id}
            className="relative flex items-center justify-end w-full group py-2"
          >
            <div
              className={`absolute right-8 flex items-center pr-2 transition-all duration-500 origin-right ${isActive ? "opacity-100 translate-x-0" : "opacity-0 translate-x-4 group-hover:opacity-100 group-hover:translate-x-0"}`}
            >
              <span
                className={`text-[10px] font-bold uppercase tracking-widest bg-background/80 px-2.5 py-1 rounded-md backdrop-blur-md border shadow-lg ${isActive ? "text-primary border-primary/20" : "text-muted-foreground border-border"}`}
              >
                {s.label}
              </span>
            </div>
            <a
              href={`#${s.id}`}
              className="block relative z-10 p-2 -mr-2 cursor-pointer"
            >
              <div
