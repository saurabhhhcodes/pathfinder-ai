...
export default function OnboardingPlanPage() {
  ...
  const renderPhase = (title, days, data, colorClass, bgClass, borderClass) => (
    <div
      className={`p-6 rounded-3xl border-2 ${borderClass} ${bgClass} shadow-sm break-inside-avoid mb-6`}
    >
      <div className="flex items-center gap-3 mb-4">
        <div
          className={`h-12 w-12 rounded-2xl ${colorClass} text-white flex items-center justify-center font-black text-xl shadow-inner`}
        >
          {days}
        </div>
        <div>
      ...
      <ul className="space-y-3 mt-6">
        {data.goals?.map((goal, idx) => (
          ...
          >
            <div
              className={`mt-1.5 h-2 w-2 rounded-full ${colorClass} shrink-0`}
            />
        ...
        >
          <div className="space-y-2">
            ...
            <p className="text-muted-foreground text-sm md:text-base font-medium">
              Generate a strategic onboarding document to hand to your new
              manager on Day 1.
            </p>
          </div>
        </motion.div>
              ...
              <div className="h-full flex items-center justify-center p-12 border-2 border-dashed border-border rounded-3xl text-center">
                <div className="max-w-md space-y-4">
                  ...
                  <p className="text-muted-foreground text-sm">
                    Enter your new job details on the left, and the AI will
                    draft a comprehensive 30-60-90 day success plan for you to
