    ...
    <div className="container max-w-6xl py-12 px-4 md:px-6">
      <div className="space-y-4 mb-10 text-center">
        ...
        <h1 className="text-4xl md:text-5xl font-black tracking-tight text-foreground">
          Protect Your{" "}
          <span className="text-gradient-primary">Energy & Peace.</span>
        </h1>
        <p className="text-lg text-muted-foreground max-w-2xl mx-auto">
          Tell us how you're feeling and what's on your plate. We'll assess your
          burnout risk and give you professional scripts to set boundaries and
          push back.
        </p>
      </div>
                ...
                {history.map((item) => (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <ListChecks
                        className={`h-4 w-4 shrink-0 ${currentAssessment?.id === item.id ? "text-rose-500" : "text-muted-foreground"}`}
                      />
            ...
            {currentAssessment ? (
              ...
              >
                ...
                <div className="flex flex-col md:flex-row gap-6 p-6 glass rounded-3xl border border-border items-center">
                  <div
                    className={`w-32 h-32 rounded-full border-8 flex items-center justify-center shrink-0 ${
                      currentAssessment.assessment.riskLevel.includes("Low")
                        ? "border-emerald-500 text-emerald-500"
                        : currentAssessment.assessment.riskLevel.includes(
                              "Moderate",
                            )
                  ...
                  >
                    <div className="text-center">
                      <span className="text-sm font-bold block mb-[-5px]">
                        Risk:
                      </span>
                      <span className="text-xl font-black uppercase">
                        {currentAssessment.assessment.riskLevel}
                      </span>
                    </div>
                  </div>
                ...
                <div className="p-6 glass rounded-3xl border border-amber-500/30 bg-amber-500/5">
                  ...
                  <ul className="space-y-3">
                    {currentAssessment.assessment.immediateActions.map(
                      (action, idx) => (
                ...
                <div className="space-y-4">
                  ...
                  {currentAssessment.assessment.scripts.map((script, idx) => (
                    <div
                      key={idx}
                      className="p-6 glass rounded-3xl border border-border group relative overflow-hidden"
                    >
                      <h4 className="font-bold text-rose-500 mb-2">
                        {script.scenario}
                      </h4>
              ...
              <div className="h-full min-h-[400px] flex flex-col items-center justify-center text-center p-8 glass rounded-3xl border border-dashed border-border">
                ...
                <p className="text-sm text-muted-foreground/60 max-w-sm mt-2">
                  Fill out the form to get personalized boundary scripts and
                  advice.
                </p>
              </div>
            )}
