...
export default function PromotionNegotiatorPage() {
  ...
  const handleGenerate = async (e) => {
    ...
    const res = await generatePromotionStrategy(
        ...
        >
          <div className="space-y-2">
            ...
            <p className="text-muted-foreground text-sm md:text-base font-medium">
              Build a bulletproof "Brag Document" and get the script to ask your
              boss for a raise.
            </p>
          </div>
        </motion.div>
          ...
          <div className="lg:col-span-5 space-y-6">
            <div className="bg-card border border-border p-6 rounded-3xl shadow-sm">
              <h3 className="font-bold text-lg mb-6 flex items-center gap-2">
                <Award className="h-5 w-5 text-purple-500" /> Your Value
                Proposition
              </h3>
              ...
              <form onSubmit={handleGenerate} className="space-y-6">
                ...
                <div className="space-y-2">
                  <label className="text-xs font-semibold text-muted-foreground uppercase tracking-wider ml-1 flex items-center gap-1.5">
                    <Award className="h-3.5 w-3.5" /> Major Achievements (Last
                    12 Months)
                  </label>
                  <Textarea
                ...
                >
                  {loading ? "Strategizing..." : "Generate Pitch"}{" "}
            ...
            {activeStrategy ? (
              ...
              >
                ...
                <div className="bg-card border border-border p-6 md:p-8 rounded-3xl shadow-md">
                  <div className="flex items-center justify-between mb-6">
                    <h3 className="text-lg font-bold flex items-center gap-2">
                      <Award className="h-5 w-5 text-purple-500" /> The Brag
                      Document
                    </h3>
                  </div>

                  <div className="space-y-6">
                    {activeStrategy.content.bragDocument?.map(
                      (section, idx) => (
                        <div
                          key={idx}
                          className="bg-muted/30 p-5 rounded-2xl border border-border"
                        >
                          <h4 className="font-bold text-purple-600 dark:text-purple-400 mb-3 uppercase tracking-wider text-sm">
                            {section.theme}
                          </h4>
                ...
                <div className="bg-card border border-border p-6 md:p-8 rounded-3xl shadow-md">
                  <div className="flex items-center justify-between mb-6">
                    <h3 className="text-lg font-bold flex items-center gap-2">
                      <MessageCircle className="h-5 w-5 text-blue-500" /> 1-on-1
                      Negotiation Script
                    </h3>
                    <Button
                      onClick={() =>
                        copyToClipboard(
                          activeStrategy.content.script.join("\n\n"),
                        )
                      }
                      variant="ghost"
                      size="sm"
                    ...
                    {activeStrategy.content.script?.map((line, idx) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          >
                            <span className="font-bold opacity-70 block mb-1 text-xs">
                              {isYou ? "You" : "Manager"}
                            </span>
              ...
              <div className="h-full flex items-center justify-center p-12 border-2 border-dashed border-border rounded-3xl text-center">
                <div className="max-w-md space-y-4">
                  ...
                  <p className="text-muted-foreground text-sm">
                    Enter your achievements from the past year. The AI will
                    calculate your value and give you the exact script to use in
                    your 1-on-1 to get that promotion.
                  </p>
                </div>
              </div>
            )}
