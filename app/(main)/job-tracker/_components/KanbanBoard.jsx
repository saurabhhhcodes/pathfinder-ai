  ...
  return (
    <div className="flex flex-col h-full space-y-4">
      ...
      <div className="flex h-full overflow-x-auto gap-6 pb-4 custom-scrollbar snap-x">
        {COLUMNS.map((column) => {
          const columnJobs = initialJobs.filter(
            (j) =>
              j.status === column.id ||
              (column.id === "Saved" && j.status === "Wishlist"),
          );
          ...
          return (
            ...
            >
              <div className="p-4 border-b border-border/50 bg-muted/30 backdrop-blur-sm flex items-center justify-between">
                <div
                  className={`px-3 py-1 text-xs font-bold uppercase tracking-wider rounded-lg border ${column.color}`}
                >
                  {column.label}
                </div>
