...
export default function LinkedInOptimizerPage() {
  ...
  const handleOptimize = async () => {
    if (content.trim().length < 50) {
      toast.error(
        "Please paste more of your profile content for an accurate analysis.",
      );
      return;
    }
        ...
        >
          <div className="space-y-2">
            ...
            <p className="text-muted-foreground text-sm md:text-base font-medium">
              Paste your LinkedIn profile text and get AI-driven improvements
              for recruiters.
            </p>
          </div>
        </motion.div>
          ...
          <div className="lg:col-span-5 space-y-6">
            <div className="bg-card border border-border p-6 rounded-3xl shadow-sm">
              ...
              <p className="text-sm text-muted-foreground mb-4">
                Copy your headline, About section, and Experience bullet points
                directly from LinkedIn and paste them here.
              </p>
              <Textarea
              ...
              >
                ...
                <div className="space-y-4">
                  <h3 className="font-bold text-lg flex items-center gap-2">
                    <CheckCircle2 className="h-5 w-5 text-primary" /> Headline
                    Suggestions
                  </h3>
                  <div className="space-y-3">
                    {activeAnalysis.headlineSuggestions?.map(
                      (headline, idx) => (
                        <div
                          key={idx}
                          className="flex items-start justify-between bg-muted/30 p-4 rounded-xl border border-border"
                        >
                ...
                <div className="space-y-4">
                  <h3 className="font-bold text-lg flex items-center gap-2">
                    <CheckCircle2 className="h-5 w-5 text-primary" /> About
                    Section
                  </h3>
                ...
                {activeAnalysis.experienceFeedback &&
                  activeAnalysis.experienceFeedback.length > 0 && (
                    <div className="space-y-4">
                      <h3 className="font-bold text-lg flex items-center gap-2">
                      ...
                      <div className="space-y-4">
                        {activeAnalysis.experienceFeedback.map((exp, idx) => (
                          <div
                            key={idx}
                            className="bg-muted/30 p-5 rounded-xl border border-border"
                          >
                ...
                <div className="space-y-4">
                  <h3 className="font-bold text-lg flex items-center gap-2">
                    <CheckCircle2 className="h-5 w-5 text-primary" /> SEO
                    Keywords to Add
                  </h3>
              ...
              <div className="h-full flex items-center justify-center p-12 border-2 border-dashed border-border rounded-3xl text-center">
                <div className="max-w-md space-y-4">
                  ...
                  <p className="text-muted-foreground text-sm">
                    Paste your profile content on the left to generate your
                    first AI-driven LinkedIn optimization report.
                  </p>
                </div>
              </div>
            )}
