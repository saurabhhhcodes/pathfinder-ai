    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center p-4 bg-background">
        ...
        <p className="text-muted-foreground mb-6">
          You haven't added any careers to compare yet.
        </p>
        <Link href="/explore">
        ...
        <div className="glass rounded-3xl border border-border/50 p-6 md:p-8 bg-card shadow-sm overflow-hidden">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
            {shortlist.map((career, i) => (
              ...
              >
                ...
                <div
                  className={`w-12 h-12 rounded-xl flex items-center justify-center mb-4 ${career.color}`}
                >
          ...
          <div className="space-y-2">
            <MetricRow
              ...
              renderValue={(c) => (
                <div className="flex items-center gap-2">
                  <span
                    className={`text-2xl font-black ${c.matchScore === highestMatch ? "text-primary" : "text-foreground"}`}
                  >
                    {c.matchScore}%
                  </span>
                  {c.matchScore === highestMatch && (
            ...
            <MetricRow
              ...
              renderValue={(c) => (
                <div className="flex items-center gap-2">
                  <span
                    className={`text-lg font-bold ${c.salaryValue === highestSalary ? "text-emerald-500" : "text-foreground"}`}
                  >
                    {c.salary}
                  </span>
                  {c.salaryValue === highestSalary && (
            ...
            <MetricRow
              ...
              renderValue={(c) => (
                <div className="flex items-center gap-2">
                  <span
                    className={`text-lg font-bold ${c.growthValue === highestGrowth ? "text-blue-500" : "text-foreground"}`}
                  >
                    {c.growth}
                  </span>
                  {c.growthValue === highestGrowth && (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-8 pt-8 border-t border-border/50">
            {shortlist.map((career) => (
              <Link
                key={`btn-${career.id}`}
                href={`/roadmap/generate?career=${encodeURIComponent(career.title)}`}
                className="w-full"
              >
                <Button
