...
} from "@/actions/internal-transfer";
...
export default function InternalTransferPage() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      const res = await generateTransferStrategy(
        currentRole,
        targetRole,
        reasons,
      );
    ...
    <div className="container max-w-6xl py-12 px-4 md:px-6">
      <div className="space-y-4 mb-10 text-center">
        ...
        <h1 className="text-4xl md:text-5xl font-black tracking-tight text-foreground">
          Nail Your{" "}
          <span className="text-gradient-primary">Internal Transfer.</span>
        </h1>
        <p className="text-lg text-muted-foreground max-w-2xl mx-auto">
          Moving teams is a delicate dance. We'll help you pitch the new hiring
          manager and gracefully break the news to your current manager without
          burning bridges.
        </p>
      </div>
                ...
                {history.map((item) => (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <ArrowRightLeft
                        className={`h-4 w-4 shrink-0 ${currentStrategy?.id === item.id ? "text-blue-500" : "text-muted-foreground"}`}
                      />
            ...
            {currentStrategy ? (
              ...
              >
                ...
                <div className="p-6 glass rounded-3xl border border-emerald-500/30 bg-emerald-500/5">
                  ...
                  <div className="space-y-4">
                    {currentStrategy.transferData.transitionPlan.map(
                      (step, idx) => (
                ...
                <div className="p-6 glass rounded-3xl border border-amber-500/30 bg-amber-500/5">
                  ...
                  <ul className="space-y-3">
                    {currentStrategy.transferData.potentialRisks.map(
                      (risk, idx) => (
              ...
              <div className="h-full min-h-[400px] flex flex-col items-center justify-center text-center p-8 glass rounded-3xl border border-dashed border-border">
                ...
                <p className="text-sm text-muted-foreground/60 max-w-sm mt-2">
                  Fill out the form to plan your internal mobility strategy
                  safely.
                </p>
              </div>
            )}
