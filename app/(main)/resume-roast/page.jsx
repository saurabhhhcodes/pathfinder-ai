...
export default function ResumeRoastPage() {
  ...
  const handleFileSelect = async (file) => {
    try {
      ...
      toast.error(
        error.message ||
          "Failed to extract text. Please try pasting it manually.",
      );
    } finally {
        ...
        >
          <div className="space-y-2">
            ...
            <p className="text-muted-foreground text-sm md:text-base font-medium">
              Get brutally honest, witty feedback from our AI hiring manager.
                  ...
                  <div className="space-y-2">
                    ...
                    {uploadSuccess && (
                      <p className="text-[10px] text-muted-foreground px-1">
                        * Please review and edit the extracted text above before
                        submitting. OCR can occasionally make mistakes.
                      </p>
                    )}
                  </div>
                </div>
              ...
              >
                ...
                <div className="space-y-4">
                  <h3 className="font-bold text-lg flex items-center gap-2">
                    <Sparkles className="h-5 w-5 text-green-500" /> How to
                    actually fix it
                  </h3>
                  <div className="space-y-3">
                    {roastData.fixes?.map((fix, idx) => (
                      <div
                        key={idx}
                        className="bg-green-500/10 p-4 rounded-xl border border-green-500/20 text-sm font-medium flex items-start gap-3"
                      >
              ...
              <div className="h-full flex items-center justify-center p-12 border-2 border-dashed border-border rounded-3xl text-center">
                <div className="max-w-md space-y-4">
                  ...
                  <p className="text-muted-foreground text-sm">
                    Paste your resume on the left and prepare for some brutally
                    honest, Gordon Ramsay-style feedback.
                  </p>
                </div>
              </div>
            )}
