...
import {
  generateExecutivePresence,
  getExecutivePresences,
} from "@/actions/executive-presence";
    ...
    <div className="container max-w-6xl py-12 px-4 md:px-6">
      <div className="space-y-4 mb-10 text-center">
        ...
        <p className="text-lg text-muted-foreground max-w-2xl mx-auto">
          Refine your professional brand, eliminate weak language, and build the
          gravitas needed for high-stakes leadership.
        </p>
      </div>
                ...
                {history.map((item) => (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <Crown
                        className={`h-4 w-4 shrink-0 ${currentPresence?.id === item.id ? "text-purple-500" : "text-muted-foreground"}`}
                      />
            ...
            {currentPresence ? (
              ...
              >
                ...
                <div className="p-6 glass rounded-3xl border border-border">
                  ...
                  <div className="space-y-4">
                    {currentPresence.presenceData.communicationUpgrades.map(
                      (upgrade, i) => (
                        <div
                          key={i}
                          className="p-4 rounded-2xl bg-background/50 border border-border relative overflow-hidden"
                        >
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 relative z-10">
                            <div>
                              ...
                              <p className="text-sm text-muted-foreground line-through decoration-rose-500/50">
                                "{upgrade.from}"
                              </p>
                            </div>
                            <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="p-6 bg-card rounded-3xl border border-indigo-500/20 shadow-sm">
                    <h4 className="font-bold text-foreground mb-4 flex items-center gap-2">
                      <Users className="h-5 w-5 text-indigo-500" /> Meeting
                      Strategy
                    </h4>
                    <ul className="space-y-3">
                      {currentPresence.presenceData.meetingStrategy.map(
                        (strategy, i) => (
                  ...
                  <div className="p-6 bg-card rounded-3xl border border-amber-500/20 shadow-sm">
                    <h4 className="font-bold text-foreground mb-4 flex items-center gap-2">
                      <Target className="h-5 w-5 text-amber-500" /> Daily
                      Gravitas Builders
                    </h4>
                    <ul className="space-y-3">
                      {currentPresence.presenceData.gravitasBuilders.map(
                        (builder, i) => (
                          <li key={i} className="flex items-start gap-2">
                            <span className="flex items-center justify-center h-5 w-5 rounded bg-amber-500/10 text-amber-500 text-xs font-bold shrink-0 mt-0.5">
                              {i + 1}
                            </span>
                            <span className="text-sm text-muted-foreground">
                              {builder}
              ...
              <div className="h-full min-h-[400px] flex flex-col items-center justify-center text-center p-8 glass rounded-3xl border border-dashed border-purple-500/30">
                ...
                <p className="text-sm text-muted-foreground/60 max-w-sm mt-2">
                  Describe your audience and leadership challenges to build your
                  executive presence.
                </p>
              </div>
            )}
