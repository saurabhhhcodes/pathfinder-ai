  ...
  return (
    ...
    >
        ...
        <Card className="glass flex flex-col items-center justify-center p-8 lg:col-span-1 border-border/50 relative overflow-hidden">
          ...
          <div className="relative w-48 h-48 flex items-center justify-center">
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center">
              ...
              <span className="text-sm font-semibold text-muted-foreground uppercase tracking-widest mt-1">
                Score
              </span>
            </div>
          </div>
                  ...
                  {matchedKeywords.length > 0 ? (
                    matchedKeywords.map((kw, i) => (
                      <Badge
                        key={i}
                        variant="outline"
                        className="bg-green-500/10 text-green-500 border-green-500/20 rounded-md"
                      >
                        {kw}
                      </Badge>
                    ))
                  ...
                  {missingKeywords.length > 0 ? (
                    missingKeywords.map((kw, i) => (
                      <Badge
                        key={i}
                        variant="outline"
                        className="bg-red-500/10 text-red-500 border-red-500/20 rounded-md"
                      >
                        {kw}
                      </Badge>
                    ))
            ...
            return (
              ...
              >
                <CardHeader className="pb-2 flex flex-row items-center justify-between space-y-0">
                  ...
                  <span className={`font-bold ${secScoreColor}`}>
                    {data.score}/100
                  </span>
                </CardHeader>
      ...
      {improvedBulletPoints?.length > 0 && (
        <div className="space-y-4">
          <h3 className="text-xl font-bold flex items-center gap-2 text-foreground">
            <Sparkles className="w-6 h-6 text-amber-500" /> Improved Bullet
            Points
          </h3>
        ...
        <Card className="glass border-border/50">
          ...
          <CardContent className="space-y-3">
            {suggestions.map((sug, i) => (
              <div
                key={i}
                className="flex gap-3 text-sm text-muted-foreground bg-background/40 p-3 rounded-lg border border-border/20"
              >
