...
export function validateInput(schema, data) {
  ...
  const preSanitizedData = JSON.parse(
    JSON.stringify(data || {}),
    (key, value) => {
...
export function parseAIJson(rawText) {
  ...
  try {
    ...
    console.error(
      "[AI JSON extraction error] Failed to parse extracted JSON:",
      error.message,
    );
    throw new Error(
      `Failed to parse JSON payload from AI response: ${error.message}`,
    );
  }
}
...
export function validateOutput(schema, rawOutput) {
  ...
  try {
    ...
    return {
      success: false,
      errors: { _output: ["AI response was not valid JSON."] },
    };
  }
