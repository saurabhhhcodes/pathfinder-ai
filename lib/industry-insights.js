...
function buildIndustryInsightPrompt(industry) {
  return buildSecurePrompt({
    ...
    untrustedData: [{ label: "industry", value: industry, maxLength: 100 }],
...
function parseInsightResponse(result) {
  ...
  if (
    !parsed ||
    !Array.isArray(parsed.salaryRanges) ||
    parsed.salaryRanges.length === 0
  ) {
...
export async function generateIndustryInsightData(industry) {
  try {
    ...
    console.error(
      `Grounded Gemini lookup failed for ${industry}, falling back to estimate:`,
      groundedError,
    );
    try {
      const payload = {
      ...
      console.error(
        "[Industry Insights] Grounding failure diagnostics:",
    ...
    try {
      ...
      console.error(
        `Gemini estimate fallback failed for ${industry}, returning default fallback:`,
        estimateError,
      );
