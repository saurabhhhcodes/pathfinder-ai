...
function hydrateCachedGeminiResult(payload) {
  ...
  const candidates = Array.isArray(payload?.responseCandidates)
    ? payload.responseCandidates
    : [];
  const response = {
    candidates,
    text: () => responseText,
...
async function readCacheValue(cacheKey) {
  try {
    return await getCacheStore().get(cacheKey);
  } catch (error) {
...
async function writeCacheValue(cacheKey, payload, ttlMs) {
  try {
    await getCacheStore().set(cacheKey, payload, ttlMs);
  } catch (error) {
    console.warn(`[cache] Cache write failed for key ${cacheKey}`, error);
  }
}

export async function cachedGenerateGeminiContent(
  prompt,
  options = {},
  cacheConfig = {},
) {
  ...
  const cacheKey =
    cacheConfig.key ?? generateCacheKey("gemini", prompt, options);
