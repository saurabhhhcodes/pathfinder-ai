  ...
  for (let index = 0; index < lastMessages.length; index += 2) {
    ...
    turns.push({
      ...
      assistant: assistantMessage
        ? normalizeValue(assistantMessage.content, 1500)
        : FALLBACK_VALUE,
    });
  }
...
export function buildIndustryContext(industry) {
  return ["Industry Context:", `- Industry: ${normalizeValue(industry)}`].join(
    "\n",
  );
}
...
export function buildUserAiContext(user, messages = []) {
  ...
  return {
    context: [profileContext, conversationContext.context]
      .filter(Boolean)
      .join("\n\n"),
    debug: {
      profileContext,
