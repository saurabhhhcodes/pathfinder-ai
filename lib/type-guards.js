...
export function isValidQuizQuestions(data) {
  ...
  if (Array.isArray(data)) {
    return interviewQuestionsOutputSchema.shape.questions.safeParse(data)
      .success;
  }
  // If it's wrapped in an object containing questions
  if (data && typeof data === "object" && Array.isArray(data.questions)) {
    return interviewQuestionsOutputSchema.shape.questions.safeParse(
      data.questions,
    ).success;
  }
...
const quizQuestionResultSchema = z
  .object({
