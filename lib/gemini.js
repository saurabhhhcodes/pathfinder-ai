...
function getGeminiModelNames() {
  ...
  return Array.from(
    new Set([configuredModel, DEFAULT_GEMINI_MODEL, ...FALLBACK_GEMINI_MODELS]),
  );
}
...
function isModelUnavailableError(error) {
  const message = [
    ...
    .filter(Boolean)
    .join(" ");
  return /404|400|invalid model|not found|not supported|not available|unsupported.*generateContent/i.test(
    message,
  );
}
...
async function withTimeout(promise, ms) {
  ...
  const timeout = new Promise((_, reject) => {
    timer = setTimeout(
      () => reject(new GeminiError("Request timed out", "TIMEOUT")),
      ms,
    );
  });
  try {
  ...
  for (const modelName of modelNames) {
    ...
    while (attempt < MAX_RETRIES) {
      try {
        ...
        if (error instanceof GeminiError && error.code === "TIMEOUT")
          throw error;
        ...
        if (status === 429 || status === 503) {
          if (attempt < MAX_RETRIES - 1) {
            ...
            console.warn(
              `[Gemini] ${status} on "${modelName}", retry ${attempt + 1} in ${Math.round(delay)}ms`,
            );
...
export async function generateGeminiContentStream(
  ...
  return runWithGeminiFallback(
    (model) => model.generateContentStream(prompt, { signal }),
    timeout,
  );
}
...
export async function generateGeminiContent(prompt, options = {}) {
  ...
  return runWithGeminiFallback((model) => {
    if (
