...
import { resumeImprovementOutputSchema as _resumeImprovementOutputSchema } from "./schemas/outputs.js";
  ...
  chat: {
    ...
    build: ({ user, userInput }) => {
      return {
        ...
        untrustedData: [
          { label: "userQuery", value: userInput.prompt, maxLength: 4000 },
        ],
      };
    },
  },
...
export async function runPrompt({
  ...
  if (!entry) {
    return {
      success: false,
      errors: { _pipeline: ["Unknown prompt pipeline type."] },
    };
  }
  ...
  const primary =
    typeof userInput === "string"
      ? userInput
      : (userInput?.prompt ?? userInput?.current ?? "");
  const guard = primary
  ...
  if (!result.success) {
    return {
      success: false,
      errors: { _output: ["AI returned an unexpected format."] },
    };
  }
