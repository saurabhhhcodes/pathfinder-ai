  ...
  return {
    ...
    async checkAndDeduct(
      ...
      if (!bucket || now >= bucket.lastRefillAt + bucketTtlMs) {
        ...
        tokens = Math.min(
          burstCapacity,
          bucket.tokens + elapsedMinutes * limitPerMinute,
        );
        lastRefillAt = now;
      }
...
export function createRateLimitStore({
  ...
  if (process.env.NODE_ENV === "production") {
    ...
    if (!redisUrl && !client) {
      throw new Error(
        "REDIS_URL is required in production when using Redis rate limiting",
      );
    }
  }
  ...
  if (process.env.NODE_ENV === "production") {
    throw new Error(
      "Rate limiting fallback to in-memory store is prohibited in production. A working Redis connection is required.",
    );
  }
  console.warn(
    "[rate-limit] Falling back to in-memory store. Set REDIS_URL for production use.",
  );
