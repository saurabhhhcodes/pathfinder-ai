...
export async function generateWithStructuredOutput({
  ...
  try {
    ...
    return {
      success: false,
      errors: {
        _output: [err?.message || "AI generation failed on first attempt."],
      },
    };
  }
  ...
  console.warn(
    "[StructuredOutput] First attempt failed validation, retrying with format correction.",
  );
  const correctionPrompt = buildFormatCorrectionPrompt(
    prompt,
    rawOutput,
    schemaDescription,
  );
  ...
  try {
    ...
    return {
      success: false,
      errors: {
        _output: [err?.message || "AI generation failed on retry attempt."],
      },
    };
  }
