...
export async function chatSalaryNegotiation(history, userMessage) {
  ...
  const prompt = buildSecurePrompt({
    context:
      "You are a tough, realistic HR representative at a tech company negotiating a salary offer with a candidate. Your goal is to get the best deal for the company, but you are willing to concede if the candidate makes strong, data-backed arguments (e.g., market rate, specific skills).",
    ...
    outputRules:
      "Keep your response concise (2-3 sentences max). Respond only as HR. Do not output JSON or markdown.",
  });
...
export async function evaluateNegotiation(history) {
  ...
  const prompt = buildSecurePrompt({
    context:
      "You are an expert career coach evaluating a salary negotiation transcript.",
