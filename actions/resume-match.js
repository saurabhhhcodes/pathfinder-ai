...
export async function analyzeResumeMatch(rawParams) {
  try {
    ...
    if (!userId) {
      return {
        success: false,
        errors: { _form: ["Sign-in required to analyze resume match."] },
      };
    }
    ...
    if (!limit.allowed) {
      return {
        ...
        errors: {
          _form: [
            `Resume match limit reached. Resets in ${formatResetTime(limit.resetAt)}.`,
          ],
        },
      };
    }
    ...
    if (!user) {
      return {
        success: false,
        errors: { _form: ["Active user account not found."] },
      };
    }
...
IMPORTANT: Return ONLY valid JSON. No markdown, no explanation outside the JSON.`,
    ...
    const record = await db.resumeMatchAnalysis.create({
      data: {
        ...
        matchScore: Math.min(
          100,
          Math.max(0, Number(parsedAnalysis.matchScore) || 0),
        ),
        matchedKeywords: Array.isArray(parsedAnalysis.matchedKeywords)
          ? parsedAnalysis.matchedKeywords.map(String)
          : [],
        missingKeywords: Array.isArray(parsedAnalysis.missingKeywords)
        ...
        suggestions: Array.isArray(parsedAnalysis.suggestions)
          ...
          : [],
        improvedBulletPoints: Array.isArray(parsedAnalysis.improvedBulletPoints)
          ? parsedAnalysis.improvedBulletPoints
          : [],
      },
    });
    ...
    return {
      success: false,
      errors: { _form: ["An error occurred processing your request"] },
    };
  }
}
