...
export async function generateResumeRoast(resumeContent) {
  ...
  if (!resumeContent || resumeContent.trim().length < 50) {
    return {
      success: false,
      errors: { _form: ["Please paste your resume content."] },
    };
  }

  const prompt = buildSecurePrompt({
    context:
      "You are the Gordon Ramsay of technical recruiting. You are brutally honest, highly critical, witty, and slightly sarcastic, but ultimately you provide actionable and deeply constructive feedback.",
    ...
    untrustedData: [{ label: "resume", value: resumeContent, maxLength: 5000 }],
    outputRules: `Provide the output in the following JSON format ONLY:
{
  ...
  try {
    ...
    return {
      success: false,
      errors: { _form: [error.message || "Failed to roast resume"] },
    };
  }
}
